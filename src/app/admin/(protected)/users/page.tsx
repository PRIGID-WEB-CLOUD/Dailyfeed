...
export default function UsersPage() {
  ...
  const [usersSnapshot, isUsersLoading, usersError] = useCollection(
  ...
  const users = useMemo(() => {
    ...
    return allUsers.filter(user => 
      !user.roles || !user.roles.some(role => adminRoles.includes(role.name))
    );
  }, [allUsers]);
  ...
  useEffect(() => {
    if (usersError) {
      toast({ variant: 'destructive', title: 'Error loading users', description: usersError.message });
    }
  }, [usersError, toast]);
  ...
  const renderPagination = () => {
    ...
    for (let i = startPage; i <= endPage, i <= totalPages; i++) { pageNumbers.push(i); }
    return (
        <Pagination>
            <PaginationContent>
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'User Removed', description: `${deletedUserName} has been removed.` });
      setSelectedUser(null);
    } catch (e: any) {
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                ...
                const userRoleName = user.roles && user.roles.length > 0 ? user.roles[0].name : 'Subscriber';
                return (
         ...
         <CardFooter className="flex items-center justify-between">
          <div className="text-xs text-muted-foreground">
             Showing <strong>{(currentPage - 1) * itemsPerPage + 1}-{(currentPage - 1) * itemsPerPage + paginatedUsers.length}</strong> of <strong>{users.length}</strong> users
          </div>
          {renderPagination()}
        </CardFooter>
      </Card>
      ...
      <Dialog open={isInviteDialogOpen} onOpenChange={setInviteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter the details of the new user. They will receive an email to set up their account.
            </DialogDescription>
          </DialogHeader>
          <AuthorForm
            roles={roles}
      ...
      <AlertDialog open={isDeleteDialogOpen} onOpenChange={(isOpen) => { setDeleteDialogOpen(isOpen); if (!isOpen) setSelectedUser(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently remove <span className="font-semibold">{selectedUser?.name}</span>'s account.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
